...
IBCC::IBCC(Parameters p, threeD_t& d, GoldLabels& g): 
  params_(p), data_(d), goldLbls_(g.lbls()) {
  cout.precision(4);
  cerr.precision(4);
  noTstEps_ = g.noTestEp_; 
  noTrEps_ = g.noTrainEp_;
  noOutputs_ = noClasses_ = g.noClasses_; 
  //noOutputs_ = noClasses_;
  unsprvd_ = params_.getParam("unsupervised") == Parameters::kTrueValue;
  noSrcs_ = data_.size();
  trainLbls_.assign(goldLbls_.begin(), goldLbls_.begin()+noTrEps_);
  cerr << "Number of sources: " << noSrcs_ << endl;
  cerr << "Number of total epochs: " << goldLbls_.size() << endl;
  cerr << "Number of training epochs: " << trainLbls_.size() << endl;
  cerr << "Number of test epochs: " << noTstEps_ << endl;
  cerr << "Number of classes: " << noClasses_ << endl;
  cerr << "Unsuperised: " << unsprvd_ << endl;
}
