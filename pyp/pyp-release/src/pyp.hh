...
class PYP : protected unordered_map<Dish, int, Hash, Hash>
{
public:
  using unordered_map<Dish,int, Hash, Hash>::const_iterator;
  using unordered_map<Dish,int, Hash, Hash>::iterator;
  using unordered_map<Dish,int, Hash, Hash>::begin;
  using unordered_map<Dish,int, Hash, Hash>::end;
  ...
  int num_types() const { return unordered_map<Dish,int, Hash, Hash>::size(); }
  bool empty() const { return _total_customers == 0; }

  double log_prob(Dish dish, double log_p0) const;
  double log_prob_minus1(Dish dish, double log_p0) const;
  // nb. d* are NOT logs
...
protected:
  ...
  typedef unordered_map<Dish, TableCounter, Hash, Hash> DishTableType;
...
: unordered_map<Dish, int, Hash, Hash>(10), _a(a), _b(b), 
...
template <typename Dish, typename Hash>
tuple<double,double,double>
PYP<Dish,Hash>::component_probs(Dish dish, double p0,
  ...
  //return boost::make_tuple(cache,new_table,denominator);
}

template <typename Dish, typename Hash>
double 
PYP<Dish,Hash>::log_prob_minus1(Dish dish, double log_p0) const
{
  using std::log;
  int c = count(dish);
  --c;
  ...
  assert(nc > 0); 
  --nc;
  double nt = num_tables();
  if((c==0) && (nt > 0)) --nt; // if c < 0 then arbitrary 
  double r = log(nt * _a + _b), prob;
  if (nc == 0) { 
    prob = log_p0;
  }
  else if (c > 0) {
    int t = num_tables(dish);
    if(t > c) {
      t=c;
    }
    prob = Log<double>::add(log(c - _a * t), r + log_p0)
      - log(nc + _b);
  }
  else {
    prob = r + log_p0 - log(nc + _b);
  }
  return prob;
}
template <typename Dish, typename Hash>
...
PYP<Dish,Hash>::log_prob(Dish dish, double log_p0) const
{
  ...
  int t = num_tables(dish);
  int nc=num_customers();
  double r = log(num_tables() * _a + _b), prob;
  if (nc == 0) { 
    prob = log_p0;
  }
  else if (c > 0) {
    ...
    prob = r + log_p0 - log(num_customers() + _b);
  }
  return prob;
}
...
PYP<Dish,Hash>::increment(Dish dish, double p0) {
  ...
  if (mt_genrand_res53() < pnew / (pshare + pnew)) {
    ...
    for (std::map<int,int>::iterator
      ...
      r -= ((hit->first - _a) * hit->second);  // why do we subtract _a from count here but not on line 467 
      if (r <= 0) {
  ...
  unordered_map<Dish,int,Hash,Hash>::operator[](dish) += 1;
  _total_customers += 1;
...
template <typename Dish, typename Hash>
double PYP<Dish,Hash>::pseudo_prob(Dish dish, double p0) {
...
//  tuple<double,double,double> components 
...
template <typename Dish, typename Hash>
double PYP<Dish,Hash>::pseudo_increment(Dish dish, double p0, double inc_count) {
  double& dcd = m_expected_dish_counts[dish]; 
  double& dtd = m_expected_dish_table_counts[dish]; 
...
template <typename Dish, typename Hash>
void PYP<Dish,Hash>::clear_expected_counts() {
  m_expected_customer_counts = 0.0;
  m_expected_table_counts = 0.0;
...
{
  typename unordered_map<Dish, int, Hash, Hash>::const_iterator dcit = find(dish);
  if (dcit != end())
    return dcit->second;
  else
    return 0;
...
{
  typename unordered_map<Dish, int, Hash, Hash>::iterator dcit = find(dish);
  if (dcit == end()) {
  ...
  typename unordered_map<Dish, TableCounter, Hash, Hash>::iterator dtit = _dish_tables.find(dish);
  ...
  for (std::map<int,int>::iterator hit = tc.table_histogram.begin();
       hit != tc.table_histogram.end(); ++hit)
  {
    r -= (hit->first) * hit->second;   // why not substract _a from first term
    if (r <= 0)
    {
      if (hit->first > 1)
        tc.table_histogram[hit->first-1] += 1;
...
{
  if(_dish_tables.size() == 0) { // nothing in restaurant yet
    return 0;
  }
  /* ERROR OCCURS HERE IN THREADED MODE WITH MANY RESTS*/
  typename DishTableType::const_iterator dtit = _dish_tables.find(dish);
...
PYP<Dish,Hash>::log_restaurant_prob() const {
  ...
  //std::cerr << std::endl;
  if(_b == 0) 
    log_prob += (_a == (double)0.0 ? (_total_tables-1.0) : (_total_tables-1.0)*log(_a) + lgamma(_total_tables-1.0));
  else 
...
// map of conditional PYPs
template<class T, class U>
class ConditionalPYPs {
public:
  ConditionalPYPs(double a, double b) 
  : m_a(a), m_b(b), m_default_restaurant(m_a, m_b) {}

  int increment(const T &x, const U &y, double P0) {
    std::pair<typename TableType::iterator, bool> result 
      = m_table.insert(std::make_pair(x, PYP<U>(m_a, m_b)));

    return result.first->second.increment(y, P0);
  }

  double expected_increment(const T &x, const U &y, double P0, double count) {
    std::pair<typename TableType::iterator, bool> result 
      = m_table.insert(std::make_pair(x, PYP<U>(m_a, m_b)));
    return result.first->second.pseudo_increment(y, P0, count);
  }

  double expected_prob(const T &x, const U &y, double P0) {
    typename TableType::iterator it = m_table.find(x);
    if (it == m_table.end())
      return m_default_restaurant.prob(y, P0);
    return it->second.pseudo_prob(y, P0);
  }

  void clear_expected_counts() {
    for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it)
      it->second.clear_expected_counts();
  }

  void clear_expected_counts(const T &x) {
    typename TableType::iterator it = m_table.find(x);
    if (it != m_table.end()) it->second.clear_expected_counts();
  }

  int decrement(const T &x, const U &y) {
    typename TableType::iterator it = m_table.find(x);
    assert (it != m_table.end());
    int delta = it->second.decrement(y);
    //if (it->second.empty())
    //  m_table.erase(it);
    return delta;
  }

  double prob(const T &x, const U &y, const double P0) {
    typename TableType::iterator it = m_table.find(x);
    if (it == m_table.end())
      return m_default_restaurant.prob(y, P0);
    return it->second.prob(y, P0);
  }

  double count(const T &x, const U &y) {
    typename TableType::iterator it = m_table.find(x);
    if (it == m_table.end())
      return m_default_restaurant.count(y);
    return it->second.count(y);
  }

  double total(const T &x) {
    typename TableType::iterator it = m_table.find(x);
    if (it == m_table.end())
      return m_default_restaurant.num_customers();
  ...
  double log_probability() {
    double llh = 0.0;
    for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it)
      llh += it->second.log_restaurant_prob();
    return llh;
  }

  double log_probability(double uniform_p0) {
    double llh = 0.0;
    for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it)
      llh += (it->second.log_restaurant_prob() + it->second.num_tables()*log(uniform_p0));
    return llh;
  }

  void resample_prior() {
    for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it)
      it->second.resample_prior(mt_genrand_res53);
  }

  void block_resample_prior() {
    int niterations=5;
    double pyp_a = a();
    double pyp_b = b();
    for (int num_its=5; num_its >= 0; --num_its) {
      resample_a_type a_log_prob(m_table);
      if (pyp_a != 0) {
        pyp_a = slice_sampler1d(a_log_prob, pyp_a, mt_genrand_res53, std::numeric_limits<double>::min(), 
                                (double) 1.0, (double) 0.0, niterations, 100*niterations);
        for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it)
          it->second.set_a(pyp_a);
      }
      if (pyp_b != 0) {
        resample_b_type b_log_prob(m_table);
        pyp_b = slice_sampler1d(b_log_prob, pyp_b, mt_genrand_res53, (double) 0.0, std::numeric_limits<double>::infinity(), 
                                (double) 0.0, niterations, 100*niterations);

        for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it)
          it->second.set_b(pyp_b);
      }
    }
  }

  double a() const { return m_table.begin()->second.a(); }
  double b() const { return m_table.begin()->second.b(); }

protected:
  struct resample_a_type {
    typedef typename ConditionalPYPs<T,U>::TableType TableType;
    TableType& m_table;
    resample_a_type(TableType& p) : m_table(p) {}

    double operator() (long double proposed_a) const {
      double log_prob=0;
      double log_prior=0;
      for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it) {
        it->second.set_a(proposed_a);
        log_prob += it->second.log_restaurant_prob();
        if (!log_prior) log_prior = it->second.log_prior();
      }
      return log_prob + log_prior;
    }
  };
  struct resample_b_type {
    typedef typename ConditionalPYPs<T,U>::TableType TableType;
    TableType& m_table;
    resample_b_type(TableType& p) : m_table(p) {}

    double operator() (long double proposed_b) const {
      double log_prob=0;
      double log_prior=0;
      for (typename TableType::iterator it=m_table.begin(); it != m_table.end(); ++it) {
        it->second.set_b(proposed_b);
        log_prob += it->second.log_restaurant_prob();
        if (!log_prior) log_prior = it->second.log_prior();
      }
      return log_prob + log_prior;
  ...
  //typedef unordered_map<T, PYP<U>, std::hash<T> > TableType;
  typedef std::map<T, PYP<U> > TableType;
  TableType m_table;

  double m_a, m_b;
  PYP<U> m_default_restaurant;
};
