...
enum State {
	...
	ST_PING_ACK,
  ST_REQ_HIST_DATA,
  ST_REQ_HIST_DATA_PROC,
  ST_REQ_HIST_DATA_ACK,
	ST_IDLE
};
...
public:
	...
	PosixTestClient(State=ST_CONNECT);
	~PosixTestClient();

	void processMessages(const IBString="");
...
public:
  ...
  twoD_t* getHistData() {return &m_histData;}
  
private:

	void reqCurrentTime();
	void placeOrder();
	void placeOrder(const IBString);
	void cancelOrder();
  void reqHistoricalData(const IBString); 
public:
	// events
...
private:
	...
	OrderId m_orderId;
  twoD_t m_histData; // date -> features
};
