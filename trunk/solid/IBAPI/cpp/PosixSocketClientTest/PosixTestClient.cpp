...
///////////////////////////////////////////////////////////
// member funcs
PosixTestClient::PosixTestClient(State st)
...
bool PosixTestClient::connect() {
  const IBString host = "127.0.0.1";
  const unsigned port = 7496;
  const int clientId = 0;
  return connect(host.c_str(), port, clientId);
}
...
void PosixTestClient::processMessages(const IBString msg)
{
	...
	switch (m_state) {
    case ST_REQ_HIST_DATA:
      reqHistoricalData(msg);
      break;
    case ST_REQ_HIST_DATA_PROC:
      break;
    case ST_REQ_HIST_DATA_ACK:
      disconnect();
      break;
		case ST_PLACEORDER:
			placeOrder(msg);
...
void PosixTestClient::orderStatus( OrderId orderId, const IBString &status, int filled,
    ...
    {
      printf( "WARNING: Cancelling order %ld\n", orderId);
